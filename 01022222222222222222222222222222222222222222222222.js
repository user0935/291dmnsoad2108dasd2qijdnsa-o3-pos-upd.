...
static isMatch() {
const supportedHostnames = \["[www.reddit.com](http://www.reddit.com)", "reddit.com", "old.reddit.com"];
...
const MAX\_LOAD\_MORE = 20;

```
while (loadMoreClicked < MAX_LOAD_MORE) {
  console.log("Scrolling post page...");
...
const MAX\_SCROLLS\_WITHOUT\_NEW\_POSTS = 5;
...
while (scrollsWithoutNewPosts < MAX_SCROLLS_WITHOUT_NEW_POSTS) {
  console.log("Finding posts on page...");
  const postLinks = await this.page.$$('shreddit-post');
  let newPostsFoundInScroll = false;

  for (const postLink of postLinks) {
    try {
      const postData = await postLink.evaluate(node => node.outerHTML);
      console.log("Post element outerHTML:", postData.slice(0, 200));

      // Attempt to get link from anchor inside the post
      const anchor = await postLink.$('a[data-click-id="body"]');
      if (anchor) {
        const postUrl = await anchor.evaluate(a => a.href);
  ...
  console.log("Scrolling subreddit page...");
  await this.utils.scroll(window, { behavior: "smooth", direction: "down" });
